...
use reth_network_api::{NetworkError, NetworkInfo,  NetworkStatus, PeerId, PeerInfo, Peers, PeersInfo};
/// A type that implements all network trait that does nothing.
///
/// Intended for testing purposes where network is not used.
#[derive(Debug, Clone, Default)]
#[non_exhaustive]
pub struct STARDUSTTESTNetwork;

impl NetworkInfo for STARDUSTTESTNetwork {
    fn local_addr(&self) -> SocketAddr {
        (IpAddr::from(std::net::Ipv4Addr::LOCALHOST), 8545).into()
    }
...
impl PeersInfo for STARDUSTTESTNetwork {
...
impl Peers for STARDUSTTESTNetwork {
