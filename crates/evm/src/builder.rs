...
#[derive(Debug)]
pub struct STARDUSTEvmBuilder<DB: Database, EXT = ()> {
...
impl<DB, EXT> STARDUSTEvmBuilder<DB, EXT>
where
    DB: Database,
{
    ...
    pub fn with_external_context<EXT1>(self, external_context: EXT1) -> STARDUSTEvmBuilder<DB, EXT1> {
        STARDUSTEvmBuilder { db: self.db, env: self.env, external_context }
    }
...
pub trait EvmFactory {
    ...
    where
        Self: Sized,
    {
        STARDUSTEvmBuilder::new(db, self.default_external_context()).build()
    }
    ...
    fn evm_with_env<'a, DB: Database + 'a>(
        ...
        STARDUSTEvmBuilder::new(db, self.default_external_context()).with_env(env.into()).build()
    }
    ...
    where
        ...
        I: GetInspector<DB>,
    {
        STARDUSTEvmBuilder::new(db, self.default_external_context())
            .with_env(env.into())
    ...
    where
        ...
        I: GetInspector<DB>,
    {
        STARDUSTEvmBuilder::new(db, self.default_external_context()).build_with_inspector(inspector)
    }
}

impl<DB: Database, EXT: Clone> EvmFactory for STARDUSTEvmBuilder<DB, EXT> {
